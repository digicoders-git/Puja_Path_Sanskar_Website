    ...
    {STEPS.map((label, i) => {
      ...
      return (
        <div key={i} className="flex-1 flex flex-col items-center relative">
          {i < STEPS.length - 1 && (
            <div className={`absolute top-4 left-1/2 w-full h-0.5 ${done ? "bg-orange-400" : "bg-gray-200"}`} style={{ zIndex: 0 }} />
          )}
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold z-10 transition-all
            ${done ? "bg-orange-500 text-white" : active ? "bg-orange-500 text-white ring-4 ring-orange-100" : "bg-gray-200 text-gray-400"}`}>
            {done ? <CheckCircle size={14} /> : num}
          </div>
          <span className={`text-xs mt-1 text-center hidden sm:block ${active ? "text-orange-500 font-semibold" : "text-gray-400"}`}>
            {label}
          </span>
        </div>
      )
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {step === 2 && (
        <div className="flex flex-col gap-8">
          ...
          <div>
            <SectionHeader title="Photo & Media Upload" />
            <div className="flex flex-col gap-4">
      ...
      {step === 3 && (
        <div className="flex flex-col gap-8">
          <div>
            <SectionHeader title="Experience & Qualification" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
      ...
      {/* Step 4: Quality + Skills + Travel + Schedule */}
      {step === 4 && (
        <div className="flex flex-col gap-8">
          <div>
            <SectionHeader title="Quality & Skill Assessment" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
          ...
          <div>
            <SectionHeader title="Availability & Travel" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
